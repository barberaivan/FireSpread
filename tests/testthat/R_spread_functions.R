...
#' @title spread_one_cell_r (documentation from c++ function)
...
spread_one_cell_r <- function(
  ...
  nd_term <- as.numeric(matrix(data_nd, nrow = 1) %*% coef_nd)

  # compute linear predictor
  linpred <-
    coef_intercepts[vegetation] +
    nd_term +
    slope_term +
    wind_term
...
#'   compute slope and wind effects). Vegetation must be the first layer, with
#'   flammability indexes in the following.
#' @param int n_fi: number of flammability indexes. These layers are located
#'   after vegetation in the landscape raster.
#' @param arma::frowvec coef_terrain: slope and wind parameters.
#' @param arma::frowvec coef_nd: coefficients for non-directional terms
#'   [intercept and flammability indexes].
...
simulate_fire_r <- function(
    ...
    det = FALSE # make the function deterministic for testing
) {
  ...
  layer_vegetation <- temp[, , 1]
  layer_nd <- temp[, , 2:3]
  layer_terrain <- temp[, , 4:n_layers]
  ...
  while(burning_size > 0 & step < steps) {
    ...
    for(b in start:end) {
      ...
      terrain_source <- layer_terrain[burning_ids[1, b], burning_ids[2, b], ]
      ...
      for(n in 1:8) {
        ...
        veg_target <- layer_vegetation[neighbours[1, n], neighbours[2, n]]
        ...
        nd_target <- layer_nd[neighbours[1, n], neighbours[2, n], ]
        terrain_target <- layer_terrain[neighbours[1, n], neighbours[2, n], ];

        # simulate fire
        burn <- spread_one_cell_r(
          veg_target+1, # to make 1-indexing
          nd_target,
          terrain_source,
          terrain_target,
          coef_intercepts,
